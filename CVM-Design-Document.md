# CVM — CUDA Version Manager: Design Document
...
### 1.2 Non-goals (v1)
The tool does **not** install or manage the **GPU kernel driver** — that stays the OS/admin's responsibility; CVM only reads it and reasons about compatibility. It does **not** manage Python framework wheels (PyTorch/TensorFlow ship their own bundled CUDA runtime — a separate axis CVM stays out of). It is **not** a container tool; Docker/NGC images are an alternative, not a target.
...
1. **Driver is system-level and backward-compatible only.** A newer driver runs older toolkits; the reverse needs the `cuda-compat` forward-compat package. Approx ceilings: driver 525→CUDA 12.0, 535→12.2, 545→12.3, 550→12.4. CVM must know this and gate switches.
...
Driver (system, not managed)  ──supports──▶  max CUDA toolkit version
        │ read via nvidia-smi
        ▼
Toolkit  (CVM-managed unit)         e.g. 12.4.1
   ├── nvcc + compiler
   ├── runtime libs (cudart, cublas, …)
   ├── headers
   └── companion libs (CVM-bundled, version-pinned):
...
Key conceptual choice (see ADR-003): the **unit of switching is a *bundle*** (toolkit + pinned cuDNN), not a bare toolkit. `cvm use 12.4` activates the toolkit *and* its associated cuDNN in one move. A bare-toolkit bundle (no cuDNN) is valid for users who only need `nvcc`.
...
└───────────────────┼──────────────────────────┼────────────────────┘
                    ...
                    │                            ▼
        ┌───────────┴────────────────────────────────────────┐
        │  cvm core  (single Go static binary)                 │
        │                                                      │
        │  ┌─────────────┐  ┌─────────────┐  ┌──────────────┐  │
        ...
        └───────────────────────┬──────────────────────────────┘
                ...
                ┌───────────────┴───────────────┐
                ▼                                ▼
      ~/.cvm  state + versions          OS package sources
...
- **Activator** — given a resolved bundle, emits an OS-appropriate env script: Unix prints `export` lines (cleaning prior CVM-injected entries first); Windows/PowerShell prints `$env:` assignments; `cmd` prints `set`. The shell shim `eval`s/invokes it.
- **Inventory** — scans known install locations to adopt existing toolkits, plus reads/writes the CVM manifest of managed bundles.
...
```
$CVM_HOME            (default ~/.cvm on Unix, %USERPROFILE%\.cvm on Windows)
├── manifest.json        # source of truth: managed bundles, adopted installs, aliases, pins
├── shims/               # cvm.sh, cvm.zsh, cvm.ps1, cvm.cmd  (shell integration)
├── versions/
...
│   │   ├── include/
│   │   ├── nvvm/
│   │   └── .cvm-meta.json   # cuDNN paired, source (adopted|downloaded), checksum, install date
│   ├── 12.4.1/
│   └── 12.6.0/
...
Adopted installs are **referenced in place** (no copy) so CVM coexists with NVIDIA's own installs.
...
```
# Inspection
cvm ls                      # installed bundles; marks active + default; shows paired cuDNN
cvm ls-remote [--cudnn]     # toolkits (and cuDNN) available to install for this OS/arch
cvm current                 # active bundle + driver + ceiling
cvm which <spec>            # absolute path of a bundle
cvm doctor                  # driver↔toolkit↔cuDNN checks + PATH lint

# Lifecycle
cvm install <ver> [--cudnn <ver>|--no-cudnn]   # download+install toolkit (+cuDNN by default)
cvm adopt [--scan]          # discover & register existing system installs
cvm uninstall <ver>
cvm cudnn install <ver> --for <toolkit>        # add/replace cuDNN in an existing bundle
cvm cudnn ls

# Activation
cvm use <spec>              # activate for THIS shell (export/$env mutation)
cvm use                     # resolve from .cuda-version in cwd (walks upward)
cvm exec <spec> -- <cmd>    # run one command under a version, no persistent switch
cvm shell <spec>            # spawn a subshell with the version active
cvm default <spec>          # persist machine/user default for new shells

# Project & aliases
cvm pin <spec>              # write .cuda-version in cwd
cvm alias <name> <ver>
cvm unalias <name>

# Plumbing (used by shim, not typed by users)
cvm env <spec> --shell <bash|zsh|powershell|cmd>   # print env script to eval
cvm hook --shell <...>                              # print cd-hook integration
```
...
The binary is invoked as `cvm env <spec> --shell <s>` and prints a script; the shim runs it in-place. Both OSes first **strip any CVM-injected entries** from the path-like vars (tracked via a `CVM_INJECTED` breadcrumb var) so repeated `use` calls don't stack duplicate paths — this mirrors nvm's `nvm_change_path` cleanup.

### 7.1 Linux / WSL / macOS
Shim `cvm.sh` is sourced from `~/.bashrc`/`~/.zshrc`. `cvm use 12.4` results in:
```sh
export CUDA_HOME=$CVM_HOME/versions/12.4.1
export CUDA_ROOT=$CUDA_HOME
export PATH=$CUDA_HOME/bin:<cleaned PATH>
export LD_LIBRARY_PATH=$CUDA_HOME/lib64:<cleaned LD_LIBRARY_PATH>
export CVM_CURRENT=12.4.1
export CVM_INJECTED=$CUDA_HOME           # breadcrumb for cleanup
```
The `cd`-hook (added via `cvm hook`) checks for `.cuda-version` on each prompt/`cd` and re-activates as needed.

### 7.2 Windows (PowerShell primary, cmd supported)
No `source` concept; the shim is a PowerShell module function (and a `.cmd` wrapper). `cvm use 12.4` (PowerShell):
```powershell
$env:CUDA_PATH = "$env:USERPROFILE\.cvm\versions\12.4.1"
$env:CUDA_HOME = $env:CUDA_PATH
$env:Path = "$env:CUDA_PATH\bin;$env:CUDA_PATH\libnvvp;" + <cleaned Path>
$env:CVM_CURRENT = "12.4.1"
$env:CVM_INJECTED = $env:CUDA_PATH
```
For a **persistent default** (`cvm default`), the core writes the *user* (not system, no admin) environment variables `CUDA_PATH`/`CUDA_PATH_V12_4` and updates a junction `…\.cvm\versions\current → 12.4.1` so tools that hard-code a fixed path still resolve. cmd users get a `set`-based script and a doskey-style `cvm` macro. A directory-change hook is provided via PowerShell's prompt function.

### 7.3 Why not just flip a symlink/junction globally?
That's the legacy `/usr/local/cuda` approach — global, often root, not per-shell or per-project. CVM offers a junction *as an opt-in compatibility shim* for hard-coded toolchains, but the default and recommended mode is per-shell env mutation (ADR-002).
...
- Each bundle records a **paired cuDNN version** in `.cvm-meta.json`. `cvm install 12.4` installs a compatible cuDNN by default (resolved from a built-in compatibility table: cuDNN 9.x ↔ CUDA 12.x, cuDNN 8.x ↔ CUDA 11.x/12.x per matrix), `--no-cudnn` opts out, `--cudnn <ver>` overrides.
...
- `cvm doctor` validates the pairing against the matrix and flags mismatches (e.g. cuDNN 8.9 sitting in a CUDA 12.6 bundle that wants 9.x).
- **Licensing reality (ADR-006):** toolkits are freely downloadable; cuDNN historically requires an NVIDIA-account download. v1 supports **user-supplied cuDNN archives** (`cvm cudnn install ./cudnn-….tar.xz --for 12.4`) and *attempts* direct download only where terms permit; otherwise it prints the exact URL and drops the user at the download page, then ingests the file. NCCL/cuBLAS-extra follow the same slot model as a stretch goal.
...
cvm install 12.4
  ...
  │     write .cvm-meta.json + update manifest.json
...
## 10. Compatibility & safety engine (`cvm doctor`)
...
- **Path hygiene** — duplicate/stale CUDA entries in `PATH`/`LD_LIBRARY_PATH`, `nvcc` on path not matching `CUDA_HOME`, leftover non-CVM CUDA exports in shell rc files.
...
**First-time setup**
```
install binary → `cvm adopt --scan` (registers existing /usr/local/cuda-* or C:\…\CUDA\v*)
   → `cvm default 12.4` → new shells start on 12.4
```
...
- **M4 — Companion libs (NCCL, cuBLAS-extra) + polish.** Shell completions, `cvm shell`, richer `ls-remote` filtering.

---

## 16. Open questions & risks
1. **Toolkit relocatability:** are NVIDIA's archive distributables fully relocatable into `$CVM_HOME`, or do some components hard-code paths? Needs a spike on Linux runfile/`.tar` and the Windows installer before M2. *(Highest-risk unknown.)*
...
4. **Naming:** "cvm" collides on GitHub (CMake VM, C/C++ VM, Composer VM, Cortex VM). Candidates: `cudaup` (rustup-style), `cudaenv`, `nvccvm`, `cuvm`.
