...
| **M4 — Companion libs + polish** | NCCL + cuBLAS-extra slots, `exec` / `shell`, shell completions, richer `ls-remote`, integration/smoke harness | WU-19 – WU-21 | 🟢 **In progress** — WU-21 (`exec`/`shell` + completions + richer `ls-remote`) landed; WU-20 NCCL discovery (registry client + `ls-remote --nccl`) landed, `nccl install` pairing pending; WU-19 (harness) pending | [Milestone #4](https://github.com/danghoangnhan/cuvm/milestone/4) |
